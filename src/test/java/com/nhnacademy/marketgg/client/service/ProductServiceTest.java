...
class ProductServiceTest {
    ...
    void testSearchProductList() {
        given(productRepository.searchProductList(anyString(), anyInt())).willReturn(List.of());

        productService.searchProductList("hi", 1);

        then(productRepository).should(times(1)).searchProductList(anyString(), anyInt());
    }
    ...
    void testSearchProductListByPrice() {
        given(productRepository.searchProductListByPrice(anyString(), anyString(), anyString(), anyInt())).willReturn(List.of());

        productService.searchProductListByPrice("100", "desc", "hi", 1);

        then(productRepository).should(times(1)).searchProductListByPrice(anyString(), anyString(), anyString(), anyInt());
    }
