...
const ClickyMenus = function (menu) {
  ...
  function closeOpenMenu(e) {
    console.warn("Uncaught DOMException: Failed to execute 'matches' on 'Element': " +
                   "'#' is not a valid selector.\nhref='#' 를 모두 제거해주세요.");
    if (currentMenuItem && !e.target.matches('#' + container.id)) {
      toggleSubmenu(currentMenuItem);
    }
  };
  ...
  function convertLinkToButton(menuItem) {
    ...
    const linkAtts = $anchor.attributes;
    const $button = document.createElement('button');

    if (null !== $anchor) {
      // copy $button attributes and content from $anchor
      ...
      menuItem.replaceChild($button, $anchor);
